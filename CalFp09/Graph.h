...
class Graph {
    ...
    void calculateShortestPath(Vertex<T> *s);
...
Edge<T> *Graph<T>::addEdge(const T &sourc, const T &dest, double capacity, double cost, double flow) {
    ...
    if (s == nullptr || d == nullptr)
        return nullptr;
    if (cost < 0)
        this->has_negative = true;
    auto *e = new Edge<T>(s, d, capacity, cost, flow);
    s->addEdge(e);
    return e;
}
...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    ...
    double total_flow = 0;
    resetFlows();
    this->calculateShortestPath(s);
    this->reduceCosts();
    while (findAugmentationPath(s, t) && total_flow < flow) {
        //cout << total_flow << endl;
        cost += t->dist;
        double f ; //= min(findMinResidualAlongPath(s, t), flow - total_flow);
        while((f = min(findMinResidualAlongPath(s, t), flow - total_flow)) > 0) {
            total_cost += f * cost;
            total_flow += f;
            augmentFlowAlongPath(s, t, f);
        }
...
void Graph<T>::reduceCosts() {
    ...
    for (Vertex<T> *v :this->vertexSet) {
        for (Edge<T> *e : v->getOutgoing()) {
            if(v->dist != INF && e->dest->dist != INF)
                e->cost += v->dist - e->dest->dist;
        }
    }

    //std::cout << "END" << std::endl;
    this->has_negative =  false;
}

template<class T>
void Graph<T>::calculateShortestPath(Vertex<T> *s) {
    if (has_negative) {
        this->bellmanFordShortestPath(s);
    } else
